...
MVC패턴을 지키지 않으면 Maassive VC가 된다. 

MVC의 대표적인 단점으로, Controller가 중재자 역할을 하다 보니 ViewController가 UI 레이아웃 설정, 비즈니스 로직, 화면 전환(Navigation), 그리고 테이블 뷰 등의 데이터 소스(DataSource) 처리까지 모든 책임을 떠안으면서 코드가 많이 증가되는 현상을 말한다.
...
| **MVVM** | ViewModel + 데이터 바인딩, View가 직접 ViewModel 관찰  |
