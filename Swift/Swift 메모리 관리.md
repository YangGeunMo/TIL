...
owner?.pet = pet      // Pet 참조 카운트 +1
pet?.owner = owner    // Owner 참조 카운트 +1

owner = nil           // 그러나 pet이 여전히 owner를 참조
pet = nil             // owner가 여전히 pet을 참조
// → 둘 다 deinit이 호출되지 않음 (메모리 누수)
```
...
class City {
    var name: String
    unowned var country: Country   // ✅ 미소유 참조 (non-Optional)
...
> City는 Country 없이 존재할 수 없으므로, City가 Country보다 오래 살 일이 없다.
> 이런 경우 `unowned`가 적합하다.
